...
Dialog::Dialog(QWidget *parent) :QDialog(parent), ui(new Ui::Dialog)
{
    ...
    connect(_serv,SIGNAL(addUserToGui(QString)),  this, SLOT(onAddUserToGui(QString)));
    connect(_serv,SIGNAL(removeUserToGui(QString)),  this, SLOT(onRemoveUserFromGui(QString)));
...
void Dialog::on_pbSend_clicked()
{
    ...
    if (!ui->cbToAll->isChecked())
        foreach (QListWidgetItem *i, ui->lwUsers->selectedItems())
            l << i->text();
    _serv->sendServerMessage(ui->teMessage->document()->toPlainText(), l);
    ui->teMessage->clear();
    if (l.isEmpty())
...
void Dialog::on_pbStartStop_toggled(bool checked)
{
    ...
    else
    {
        ...
        if (_serv->startServer(addr, ui->lePort->text().toInt()))
        {
